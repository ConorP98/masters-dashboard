...
def load_players():
    ...
    for col in pct_cols:
        if col in df.columns and pd.to_numeric(df[col], errors="coerce").dropna().max() <= 1.0:
            df[col] = pd.to_numeric(df[col], errors="coerce") * 100
    # Exclude withdrawn/ineligible players
...
def render_sidebar(df):
    with st.sidebar:
        ...
        if st.session_state.page == "Player Picker":
            ...
            odds_range=(min(odds_lo,odds_hi),max(odds_lo,odds_hi)); st.session_state.filter_odds_range=odds_range
            ...
            age_range=(min(age_lo,age_hi),max(age_lo,age_hi)); st.session_state.filter_age_range=age_range
            ...
            cuts_range=(min(cuts_lo,cuts_hi),max(cuts_lo,cuts_hi)); st.session_state.filter_cuts_range=cuts_range
...
def render_player_picker(df, filters):
    ...
    with left_col:
        ...
        sort_col = st.selectbox("Sort by",list(SORT_ASC.keys()),format_func=lambda x:x.replace("_"," ").title(),label_visibility="collapsed")
...
def _render_profile_overview(row, df):
    ...
    with col1:
        ...
        for field,label in [("college","COLLEGE"),("height","HEIGHT"),("weight","WEIGHT"),("residence","RESIDENCE")]:
            v = row.get(field)
        ...
        if insta and str(insta) not in ["","nan","None"]:
            ...
            st.markdown(f"""<div class="bio-card" style="padding:12px 14px;margin-top:0;">
                <a href="https://instagram.com/{handle}" target="_blank" style="display:flex;align-items:center;gap:10px;text-decoration:none;color:#ffffff;">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <rect x="2" y="2" width="20" height="20" rx="5" ry="5" stroke="url(#ig)" stroke-width="2"/>
...
def _render_profile_combined(row, df):
    ...
    with col1:
        ...
        for label,col_name,fmt_fn,highlight in masters_stats:
            ...
            if val is not None and not (isinstance(val,float) and np.isnan(val)) and str(val) not in ["","nan","None"]:
                try:
                    display = fmt_fn(val)
                    cls = "highlight" if highlight else ""
    ...
    with col2:
        ...
        fig = go.Figure()
        fig.add_trace(go.Histogram(x=df["odds"],nbinsx=25,marker_color="rgba(74,222,128,0.25)",
...
def _render_profile_radar(row, df):
    ...
    # Only use norm cols whose raw column is numeric
    valid_norm_cols = []
    for nc in norm_cols:
        raw = nc[:-5]
        if raw in df.columns:
            if pd.to_numeric(df[raw], errors="coerce").notna().any():
                valid_norm_cols.append(nc)
        else:
            valid_norm_cols.append(nc)
    norm_cols = valid_norm_cols
    if not norm_cols:
        st.info("No usable radar columns found.")
        return
    ...
    if compare_label == "Field Average":
        ...
        compare_raw_vals = []
        for nc in norm_cols:
            compare_vals.append(safe_float(df[nc].mean()))
            rc = raw_col_map[nc]
            col_data = pd.to_numeric(df[rc] if rc in df.columns else df[nc], errors="coerce")
            avg = col_data.mean()
            compare_raw_vals.append(f"{avg:.2f}" if pd.notna(avg) else "—")
    ...
    fig.update_layout(
        polar=dict(bgcolor="rgba(17,24,17,0)",
            radialaxis=dict(visible=True,range=[0,1],gridcolor="rgba(74,222,128,0.15)",
                linecolor="rgba(74,222,128,0.15)",tickfont=dict(color="#a0b8a0",size=9,family="DM Mono")),
...
Sum of each player's odds (e.g. 12 + 50 + 100 = 162). Must be ≥ 150 for a valid entry.
    ...
    with row1_l:
        ...
        fig.update_layout(paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(17,24,17,0.4)",
            ...
            margin=dict(l=40,r=20,t=30,b=40),height=380,legend=dict(bgcolor="rgba(0,0,0,0)",font=dict(size=11,color="#ffffff")))
        st.plotly_chart(fig,use_container_width=True)
    with row1_r:
        ...
        fig = go.Figure(go.Bar(x=risk_counts["Risk"],y=risk_counts["Count"],
            marker_color=["#4ade80","#fbbf24","#f87171"],text=risk_counts["Count"],
    ...
    with row2_l:
        ...
        fig.update_layout(paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(17,24,17,0.4)",
            ...
            margin=dict(l=40,r=20,t=30,b=40),height=360,legend=dict(bgcolor="rgba(0,0,0,0)",font=dict(size=11,color="#ffffff")))
        st.plotly_chart(fig,use_container_width=True)
    with row2_r:
        ...
        st.dataframe(top20.reset_index(drop=True),use_container_width=True,height=360,
            column_config={"Player":st.column_config.TextColumn("Player"),"Odds":st.column_config.TextColumn("Odds"),
    ...
    st.dataframe(tdf.reset_index(drop=True),use_container_width=True,height=500,
        column_config={"Player":st.column_config.TextColumn("Player"),"country":st.column_config.TextColumn("Country"),
...
def render_historical(hist_odds, hist_picks, hist_winners, hist_scores, hist_rounds, hist_teams):
    ...
    if not hist_picks.empty:
        name_col = next((c for c in ["full_name","name","player"] if c in hist_picks.columns), None)
        first_col = "first" if "first" in hist_picks.columns else None
        last_col  = "last"  if "last"  in hist_picks.columns else None
        picks_col = next((c for c in ["num_picks","picks","count"] if c in hist_picks.columns), None)

        if first_col and last_col:
            hist_picks["_name"] = hist_picks[first_col]+" "+hist_picks[last_col]
        elif name_col:
            hist_picks["_name"] = hist_picks[name_col]
        else:
        ...
        if picks_col:
            ...
            fig = go.Figure(go.Bar(x=agg[picks_col],y=agg["_name"],orientation="h",
                marker=dict(color=agg[picks_col],colorscale=[[0,"#166534"],[1,"#4ade80"]],showscale=False),
                hovertemplate="<b>%{y}</b><br>Total picks: %{x}<extra></extra>"))
            fig.update_layout(**_chart_layout(400,yaxis={"autorange":"reversed"}))
            st.plotly_chart(fig,use_container_width=True)
        else:
            # Just count rows
            agg = hist_picks["_name"].value_counts().reset_index()
            agg.columns = ["Player","Times Picked"]
            agg = agg.head(15)
            fig = go.Figure(go.Bar(x=agg["Times Picked"],y=agg["Player"],orientation="h",
    ...
    # ── Most Selected Player by Year (with year filter) ──
    ...
    if not hist_picks.empty and "year" in hist_picks.columns:
        ...
        sel_year_top = st.selectbox("Year",["All"]+[str(y) for y in years_avail],key="top_player_year")
        pdf = hist_picks if sel_year_top=="All" else hist_picks[hist_picks["year"]==int(sel_year_top)]
        if not pdf.empty:
            picks_col = next((c for c in ["num_picks","picks","count"] if c in pdf.columns),None)
            if picks_col:
                pdf[picks_col] = pd.to_numeric(pdf[picks_col],errors="coerce")
                agg2 = pdf.groupby("_name",as_index=False)[picks_col].sum().sort_values(picks_col,ascending=False).head(10)
            else:
                agg2 = pdf["_name"].value_counts().reset_index()
                agg2.columns = ["_name","count"]
                picks_col = "count"
                agg2 = agg2.head(10)
            # Get odds from hist_odds if available
            odds_col = None
            if not hist_odds.empty and "odds" in hist_odds.columns:
                odds_col = "odds"
                odds_map = hist_odds.set_index("_name")["odds"].to_dict() if "_name" in hist_odds.columns else {}
                agg2["Odds"] = agg2["_name"].map(odds_map).fillna("—")
            cols_show = ["_name",picks_col]
            if odds_col: cols_show.append("Odds")
            agg2 = agg2.rename(columns={"_name":"Player",picks_col:"Times Picked"})
            st.dataframe(agg2.reset_index(drop=True),use_container_width=True,height=300,hide_index=True)
        else:
    ...
    # ── Historical Picks by Contestant ──
    st.markdown("**Picks by Contestant**")
    if not hist_picks.empty:
        contestant_col = next((c for c in ["participant","contestant","entered_by","user","name"] if c in hist_picks.columns),None)
        year_col_p = "year" if "year" in hist_picks.columns else None
        if contestant_col:
            years_c = sorted(hist_picks["year"].dropna().unique().astype(int).tolist()) if year_col_p else []
            sel_year_c = st.selectbox("Year",["All"]+[str(y) for y in years_c],key="contestant_year")
            cdf = hist_picks if sel_year_c=="All" else hist_picks[hist_picks["year"]==int(sel_year_c)]
            contestants = sorted(cdf[contestant_col].dropna().unique().tolist())
            sel_contestant = st.selectbox("Contestant",contestants,key="contestant_sel") if contestants else None
            if sel_contestant:
                player_picks = cdf[cdf[contestant_col]==sel_contestant][["_name"]+(["year"] if year_col_p else [])]
                player_picks = player_picks.rename(columns={"_name":"Player"})
                st.dataframe(player_picks.reset_index(drop=True),use_container_width=True,height=250,hide_index=True)
            else:
                st.info("No contestants found.")
        else:
    ...
    # ── Teams entered per year: cut/missed cut ──
    ...
    if not hist_teams.empty and "year" in hist_teams.columns:
        ...
        cut_col = next((c for c in ["made_cut","cut","cut_status","made_the_cut"] if c in hist_teams.columns),None)
        if cut_col:
            ...
            fig = go.Figure()
            fig.add_trace(go.Bar(name="Made Cut",x=[str(int(y)) for y in years_u],
                y=[made.get(y,0) for y in years_u],marker_color="#4ade80"))
            fig.add_trace(go.Bar(name="Missed Cut",x=[str(int(y)) for y in years_u],
                y=[missed.get(y,0) for y in years_u],marker_color="#f87171"))
            fig.update_layout(**_chart_layout(340,extra={"barmode":"stack",
                "legend":dict(bgcolor="rgba(0,0,0,0)",font=dict(color="#ffffff",size=11))}))
            st.plotly_chart(fig,use_container_width=True)
        else:
            # Just show count per year
            per_year = hist_teams.groupby("year").size().reset_index(name="Teams")
            fig = go.Figure(go.Bar(x=per_year["year"].astype(int).astype(str),y=per_year["Teams"],
                marker_color="#4ade80",text=per_year["Teams"],textposition="outside"))
            fig.update_layout(**_chart_layout(300,xaxis={"title":"Year"},yaxis={"title":"Teams"}))
            st.plotly_chart(fig,use_container_width=True)
    else:
    ...
    if not hist_winners.empty:
        rename_map = {}
        if "participant" in hist_winners.columns:
            rename_map["participant"] = "Sweepstakes Winner"
        if "full_name" in hist_winners.columns:
            rename_map["full_name"] = "Masters Winner"
        if "year" in hist_winners.columns:
